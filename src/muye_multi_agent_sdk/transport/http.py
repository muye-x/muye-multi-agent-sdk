...
def create_app(
    ...
    *,
    ...
    """创建标准 Agent ASGI 应用；默认不允许浏览器跨域访问。"""
    ...
    if "internal" in agent.config.api.profiles:
        ...
        async def invoke(request: AgentRequest) -> dict[str, Any]:
            return _internal_response(await agent.invoke(request, options=ExecutionOptions(profile="internal", context_enabled="internal" in agent.config.context.enabled_profiles)))

        @app.post("/invoke/stream")
        async def invoke_stream(request: AgentRequest) -> StreamingResponse:
            return _streaming_response(agent, request, profile="internal")
    ...
    if "public" in agent.config.api.profiles:
        ...
        async def public_stream(payload: dict[str, Any]) -> StreamingResponse:
            return _streaming_response(agent, _public_request(payload, public_request_adapter), profile="public")
...
def _public_response(result: AgentResult) -> dict[str, Any]:
    ...
    return {"status": "success", "markdown": markdown, "chart": result_data.get("chart") or result_data.get("charts"), "json_data": result_data.get("json_data"), "trace_id": result.trace_id}


def _streaming_response(agent: BaseAgent, request: AgentRequest, *, profile: str) -> StreamingResponse:
    async def source():
        emitter = SseEmitter(session_id=request.context.session_id, user_id=request.context.user_id)
        yield emitter.session_start()
        try:
            options = ExecutionOptions(profile=profile, context_enabled=profile in agent.config.context.enabled_profiles, public_route=profile == "public")
